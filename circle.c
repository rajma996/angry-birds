...
VAO* create_circle(float x,float y,float a,float b,float angle,float red,float green , float blue,float vx,float vy)
{
  ...
  vao->rotate_angle=angle; vao->starttime=glfwGetTime();vao->vx=vx; vao->vy=vy;
  return vao;
}
...
VAO* create_bubble(float x,float y,float a,float b,float angle,float red,float green , float blue,float vx,float vy)
{
  ...
  {
    color_buffer_data[j++]=red;color_buffer_data[j++]=green;color_buffer_data[j++]=blue;
    color_buffer_data[j++]=red-0.65;color_buffer_data[j++]=green-0.65;color_buffer_data[j++]=blue-0.65;
    color_buffer_data[j++]=red;color_buffer_data[j++]=green;color_buffer_data[j++]=blue;
  }
  struct VAO* vao = new struct VAO;
  vao = create3DObject(GL_TRIANGLES, 216 , vertex_buffer_data, color_buffer_data, GL_FILL);
